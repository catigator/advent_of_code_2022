...
def calculate_possible_moves(matrix, part=1):
    ylen = len(matrix)
    xlen = len(matrix[0])
            ...
            for key, move in MOVES.items():
                ...
                if in_range(new_pos, matrix):
                    if part == 1:
                        ...
                        possible_move = is_possible_move_2(pos, new_pos, matrix)
                    if possible_move:
...
def get_start_pos(matrix) -> Optional[Tuple]:
    return get_pos_for_char(matrix, "S")[0]


def get_end_pos(matrix) -> Optional[Tuple]:
    return get_pos_for_char(matrix, "E")[0]
...
def is_possible_move_2(a, b, matrix):
    value_a = get_value(matrix[a[0]][a[1]])
    value_b = get_value(matrix[b[0]][b[1]])
...
def solve_part_2():
    print("Day 12 - Part 2")
    lines = read_input_lines(INPUT_FILENAME, True)
    s_pos = get_start_pos(lines)
    start_positions = get_pos_for_char(lines, "a")
